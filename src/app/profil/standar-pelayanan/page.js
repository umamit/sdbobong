...
export default function StandarPelayananPage() {
  ...
  useEffect(() => {
    fetch('/api/config')
      .then(res => res.json())
      .then(data => {
        const list = data?.config?.stats?.page_contents?.profil?.standar_pelayanan_list || [];
        setServices(list);
        setLoading(false);
      })
  ...
  const activeService = (services.length > 0 && services[activeIdx]) ? services[activeIdx] : {
  ...
  return (
    <div style={{ backgroundColor: 'var(--bg-main)', minHeight: '100vh', padding: 'var(--space-xl) 0' }}>
      <div className="container" style={{ maxWidth: '800px', margin: '0 auto', padding: '0 var(--space-md)' }}>
        ...
        {services.length > 1 && (
          <div style={{ display: 'flex', gap: '8px', overflowX: 'auto', paddingBottom: '12px', marginBottom: 'var(--space-md)', scrollbarWidth: 'thin' }}>
