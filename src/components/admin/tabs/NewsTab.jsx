...
import AIDraftGenerator from './news/AIDraftGenerator';
import AIDraftModal from './news/AIDraftModal';
...
export default function NewsTab() {
  ...
  const handleGenerateDraft = async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(data.error || 'Gagal menghasilkan draf berita.');
  ...
  const handleApplyDraft = () => {
    ...
    if (titleInput) titleInput.value = generatedDraft.title;

    const categoryInput = document.getElementById('news_category');
    if (categoryInput) categoryInput.value = generatedDraft.category;

    const dateInput = document.getElementById('news_date');
    if (dateInput) dateInput.value = generatedDraft.date;
  ...
  return (
    <section id="tab-news" className={`tab-pane ${activeTab === 'news' ? 'active' : ''}`}>
      <div className="news-cms-grid news-stacked-layout">
        ...
        {/* Asisten AI Pembuat Draf Berita */}
        {!editingNews && (
          <AIDraftGenerator
            aiPrompt={aiPrompt}
        ...
        <div id="news_form_section" className="settings-card">
          ...
          <form onSubmit={editingNews ? handleNewsUpdate : handleNewsAdd} key={editingNews ? `edit-${editingNews.id}-${newsEditorKey}` : 'new'} encType="multipart/form-data">
            <div className="form-group" style={{ marginBottom: 'var(--space-sm)' }}>
              ...
              <input type="text" id="news_title" name="title" className="form-control" placeholder="Contoh: Pembagian Rapor Semester Genap" defaultValue={editingNews ? editingNews.title : ''} style={{ width: '100%' }} required />
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'var(--space-md)', marginBottom: 'var(--space-sm)' }}>
              <div className="form-group">
                ...
                <input type="text" id="news_date" name="date" className="form-control" placeholder="Contoh: 20 Jun 2026" defaultValue={editingNews ? editingNews.date : ''} style={{ width: '100%' }} required />
              </div>
              <div className="form-group">
                <label htmlFor="news_category" style={{ display: 'block', marginBottom: '4px', fontWeight: 600 }}>Kategori *</label>
                <input type="text" id="news_category" name="category" className="form-control" placeholder="Contoh: Kegiatan, Prestasi" defaultValue={editingNews ? editingNews.category : ''} style={{ width: '100%' }} required />
              </div>
            </div>
                  ...
                  <div style={{ marginTop: '12px', padding: '12px', borderRadius: '8px', background: 'rgba(255, 255, 255, 0.05)', border: '1px solid rgba(255, 255, 255, 0.1)', backdropFilter: 'blur(10px)', WebkitBackdropFilter: 'blur(10px)', boxShadow: '0 4px 15px rgba(0, 0, 0, 0.1)' }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(75px, 1fr))', gap: '8px' }}>
                      {newsPhotoPreviews.map((previewUrl, index) => (
                        <div key={index} style={{ position: 'relative', aspectRatio: '1/1', borderRadius: '6px', overflow: 'hidden', border: '1px solid rgba(255, 255, 255, 0.15)', boxShadow: '0 2px 8px rgba(0,0,0,0.15)' }}>
                          <img src={previewUrl} alt={`Preview ${index}`} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                          <button
                            type="button"
                            onClick={() => handleRemoveNewsPhoto(index)}
                            style={{ position: 'absolute', top: '4px', right: '4px', width: '18px', height: '18px', borderRadius: '50%', background: 'rgba(239, 68, 68, 0.9)', color: '#ffffff', border: 'none', fontSize: '10px', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', boxShadow: '0 1px 3px rgba(0,0,0,0.3)', transition: 'background 0.2s, transform 0.2s' }}
            ...
            <div className="form-group" style={{ marginBottom: 'var(--space-sm)' }}>
              ...
              <RichTextEditor key={newsEditorKey} defaultValue={editingNews ? editingNews.content : ''} placeholder="Tuliskan isi berita lengkap di sini..." />
            </div>
              ...
              newsList.map((n) => (
                <div key={n.id} style={{ display: 'flex', gap: 'var(--space-sm)', border: '1px solid var(--border-color)', padding: 'var(--space-xs)', borderRadius: 'var(--radius-sm)', alignItems: 'center', justifyContent: 'space-between', backgroundColor: 'var(--bg-main)' }}>
                  <div style={{ display: 'flex', gap: 'var(--space-xs)', alignItems: 'center', minWidth: 0 }}>
                    <img src={n.image} alt="" width="50" height="50" style={{ borderRadius: 'var(--radius-sm)', objectFit: 'cover', flexShrink: 0, border: '1px solid var(--border-color)' }} loading="lazy" />
