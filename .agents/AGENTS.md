...
* **Never Mix Server and Client in One File:** Since the project uses pure JSX, explicitly enforce that files containing client-side interactivity (e.g., `useState`, `useEffect`, `onClick`) MUST include the `"use client"` directive at the very top. Conversely, handlers or queries fetching sensitive DB information must remain strictly on the server-side.
* **Root Configuration Protection:** Never automatically modify `layout.js`, `middleware.js`, `next.config.js`, or `package.json` unless the user explicitly targets those paths.
